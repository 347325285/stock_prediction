...
with tf.Session() as sess:
    ...
    for i in range(len(x1)-(given+wanted)+1):
        ...
        temp_m[[i],:] = [0]*(i+given) + list(sess.run(predict, feed_dict=feed_dict)[0]) + [0]*(len(x1)-(given+wanted)-i)
...
with tf.Session() as sess:
    ...
    for i in range(len(x1)-(given+wanted)+1):
        ...
        feed_dict = {
        X: np.array(y[start:(start+given),:]).reshape([1, -1]),
        Y: np.array(y1[(start+given+1):(start+given+1+wanted)]).reshape([1, -1])
        }
